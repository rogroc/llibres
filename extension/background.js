...
// --------------------------------------------------------------------------
// UTILITAT DE DIAGNÒSTIC (LOGS AL SERVIDOR)
// --------------------------------------------------------------------------

async function logToServer(sid, msg, level = 'log') {
    console.log(`[BG] [${sid}] ${msg}`);
    try {
        await fetch('http://localhost:8080/api/log', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ sid, level, msg: `[BG] ${msg}` })
        });
    } catch (_) {}
}
...
function openIntranetTab(sid, shouldFocus = false, pageType = 'registre', callback) {
    if (!sid) sid = 'default';
    logToServer(sid, `openIntranetTab cridat (shouldFocus=${shouldFocus}, pageType=${pageType})`);
    
    // Si callback és una funció i no s'ha passat pageType, ajustem els paràmetres
    if (typeof pageType === 'function') {
        callback = pageType;
        pageType = 'registre';
    }
    
    const pagePath = pageType === 'buscador' ? 'buscador' : 'registre';
    const targetUrl = pageType === 'buscador' 
        ? `https://www.llibreviu.org/admin/buscador?sid=${sid}` 
        : `https://www.llibreviu.org/admin/registre/?sid=${sid}`;
    
    chrome.tabs.query({}, (allTabs) => {
        logToServer(sid, `chrome.tabs.query ha trobat ${allTabs.length} pestanyes obertes.`);
        // Busquem qualsevol pestanya d'administració de llibreviu per a aquest sid
        const intranetTab = allTabs.find(tab => {
            if (!tab.url) return false;
            const urlStr = tab.url.toLowerCase();
            return urlStr.includes('llibreviu.org/admin/') && urlStr.includes('sid=' + sid.toLowerCase());
        });
        
        if (intranetTab) {
            logToServer(sid, `Pestanya de la intranet trobada (ID: ${intranetTab.id}, URL: ${intranetTab.url}).`);
            const urlStr = intranetTab.url.toLowerCase();
            // Comprovem si ja és a la pàgina demanada (admetent registre o registres per a registre)
            const hasCorrectPath = urlStr.includes(`llibreviu.org/admin/${pagePath}`) || 
                                   (pagePath === 'registre' && urlStr.includes('llibreviu.org/admin/registres'));
            
            if (!hasCorrectPath) {
                logToServer(sid, `Redirigint pestanya existent a ${targetUrl} per canvi de pantalla...`);
                chrome.tabs.update(intranetTab.id, { url: targetUrl, active: shouldFocus }, () => {
                    if (shouldFocus) {
                        chrome.windows.update(intranetTab.windowId, { drawAttention: true, focused: true }, () => {
                            logToServer(sid, `Pestanya redirigida i enfocada.`);
                        });
                    }
                ...
                if (shouldFocus) {
                    logToServer(sid, `Enfocant pestanya existent de forma directa...`);
                    chrome.tabs.update(intranetTab.id, { active: true }, () => {
                        chrome.windows.update(intranetTab.windowId, { drawAttention: true, focused: true }, () => {
                            logToServer(sid, `Pestanya existent enfocada amb èxit.`);
                        });
                    });
                }
                if (callback) callback({ success: true, opened: false });
            ...
            // Si no hi ha cap pestanya amb sid, busquem una genèrica de llibreviu admin sense sid
            const genericTab = allTabs.find(tab => {
                if (!tab.url) return false;
                const urlStr = tab.url.toLowerCase();
                return urlStr.includes('llibreviu.org/admin/') && !urlStr.includes('sid=');
            });
            if (genericTab) {
                logToServer(sid, `Trobada una pestanya genèrica sense sid. Re-utilitzant-la i redirigint a targetUrl...`);
                chrome.tabs.update(genericTab.id, { url: targetUrl, active: shouldFocus }, () => {
                    if (shouldFocus) {
                        chrome.windows.update(genericTab.windowId, { drawAttention: true, focused: true }, () => {
                            logToServer(sid, `Pestanya genèrica enfocada.`);
                        });
                    }
                    if (callback) callback({ success: true, opened: false });
                });
                ...
                if (now - lastOpen < 4000) {
                    logToServer(sid, `Obertura de pestanya bloquejada per anti-bounce (fa menys de 4s).`);
                    if (callback) callback({ success: false, error: 'Debounced' });
                    return;
                }

                // Si som l'extensió de Chrome real i no tenim cap pestanya d'aquesta sessió (sid),
                // significa que la sessió s'està gestionant des de l'aplicació d'Electron.
                // Per tant, bloquegem l'obertura de noves pestanyes a Chrome per no molestar l'usuari.
                const isElectron = chrome.runtime.id === 'llibreviu-desktop-mock';
                if (!isElectron) {
                    console.log(`[BG] Ignorant creació de pestanya per a sid: ${sid} a Chrome (gestionat per Electron)`);
                    if (callback) callback({ success: false, error: 'Managed by Electron' });
                    return;
                }

                logToServer(sid, `Cap pestanya de la intranet trobada. Creant-ne una de nova per a sid...`);
                lastIntranetOpenTimes[sid] = now;
                chrome.tabs.create({ url: targetUrl, active: shouldFocus }, (newTab) => {
                    logToServer(sid, `Nova pestanya creada (ID: ${newTab ? newTab.id : 'desconegut'}).`);
...
async function searchCatalogsByIsbn(isbn) {
    ...
    // Afegeix portades d'AbeBooks/BookFinder com a fallback si no en tenen (o tenen el proxy genèric de syndetics)
    await Promise.all(filtered.map(async (c) => {
        if (c.isbn && (!c.cover || c.cover.includes('blank.gif') || c.cover.includes('no-image') || c.cover.includes('no_image') || c.cover.includes('syndetics.com'))) {
            const resolved = await resolveCandidateCover(c.isbn);
            if (resolved) {
                c.cover = resolved;
            }
        }
...
async function resolveCandidateCover(isbn) {
    if (!isbn) return null;
    ...
    try {
        ...
        if (res.ok) {
            ...
            while ((imgMatch = imgRegex.exec(html)) !== null) {
                ...
                if (src.toLowerCase().includes('cover') || src.toLowerCase().includes('isbn') || src.includes(cleanIsbn)) {
                    return src;
                }
            }
        }
    ...
    // 3. Open Library fallback (si és de veritat de >1KB)
...
// --------------------------------------------------------------------------
// CANAL DE COMUNICACIÓ PERSISTENT (PORT) PER A ESDEVENIMENTS I KEEPALIVE
// --------------------------------------------------------------------------

const activePorts = new Map(); // port -> sid
let globalSse = null;

function connectGlobalSse() {
    if (globalSse) globalSse.close();
    
    const sseUrl = `http://localhost:8080/api/sse?sid=global`;
    console.log(`[BG] 🔌 Obrint SSE de fons global a: ${sseUrl}`);
    globalSse = new EventSource(sseUrl);

    globalSse.addEventListener('state', async (event) => {
        try {
            const stateData = JSON.parse(event.data);
            const sid = stateData.sid;
            if (!sid) return;

            console.log(`[BG] [SSE:global] Rebut estat per a ${sid}: ${stateData.state}`);

            // Envia l'estat a qualsevol port actiu associat a aquest sid
            for (const [port, portSid] of activePorts.entries()) {
                if (portSid === sid) {
                    port.postMessage({ action: 'state_update', data: stateData });
                }
            }

            // Si s'ha d'executar una cerca d'OCR o catàlegs
            if (stateData.state === 'searching' && !processingSearchSids.has(sid)) {
                processSearchForSession(sid);
            }

            // Si el llibre està llest per injectar, o s'està editant/desant, ens assegurem que s'obre/enfoca
            if (['filling', 'editing', 'saving'].includes(stateData.state)) {
                openIntranetTab(sid, true);
            }

            // Si hi ha activitat de baixa en curs, ens assegurem que la pestanya de l'admin/buscador està en primer pla
            if (stateData.decommission_status === 'searching' || stateData.decommission_status === 'decommissioning') {
                openIntranetTab(sid, true, 'buscador');
            }
        } catch (err) {
            console.error(`[BG] [SSE:global] Error parsing state:`, err);
        }
    });

    globalSse.onerror = () => {
        console.warn(`[BG] [SSE:global] Error de connexió SSE global. Reconnectant en 3s...`);
        globalSse.close();
        globalSse = null;
...
// Inicialitzem la connexió SSE global immediatament en arrencar el Service Worker
connectGlobalSse();

chrome.runtime.onConnect.addListener((port) => {
    if (port.name !== 'llibreviu-sync') return;
    console.log(`[BG] [Port] Client intranet connectat.`);

    port.onMessage.addListener(async (msg) => {
        if (msg.action === 'init') {
            const sid = msg.sid;
            console.log(`[BG] [Port] Inicialitzat amb sid: ${sid}`);
            activePorts.set(port, sid);

            // De forma immediata, demanem l'estat actual al servidor per si la pàgina
            // s'ha acabat de carregar després que s'hagi enviat la petició d'omplir
            fetch(`http://localhost:8080/api/session-state?sid=${sid}&t=${Date.now()}`)
                .then(res => res.json())
                .then(stateData => {
                    logToServer(sid, `Port inicialitzat. Estat actual del servidor: ${stateData.state}`);
                    if (stateData && stateData.state) {
                        logToServer(sid, `Retransmetent estat inicial de càrrega (${stateData.state}) a la pestanya.`);
                        port.postMessage({ action: 'state_update', data: stateData });
                    }
                })
                .catch(err => {
                    console.error(`[BG] Error obtenint estat inicial per a ${sid}:`, err);
                });
            return;
        }

        if (msg.action === 'ping') {
            // Heartbeat/batec buit per evitar la suspensió automàtica del Worker
            port.postMessage({ action: 'pong' });
            return;
        }

        if (msg.action === 'process_search') {
            const sid = msg.sid;
            if (!processingSearchSids.has(sid)) {
                openIntranetTab(sid, true, () => {
                    processSearchForSession(sid);
        ...
        if (msg.action === 'focus_tab') {
            const sid = msg.sid;
            // Netegem la sol·licitud d'enfocar pestanya al servidor
    ...
    port.onDisconnect.addListener(() => {
        const portSid = activePorts.get(port);
        console.log(`[BG] [Port] Client intranet desconnectat (sid: ${portSid}).`);
...
chrome.runtime.onInstalled.addListener((details) => {
    if (details.reason === 'install') {
        chrome.tabs.create({ url: 'https://www.llibreviu.org/admin/registre/' });
    }
