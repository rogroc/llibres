...
function openIntranetTab(sid, shouldFocus = false, callback) {
    if (!sid) sid = 'default';
    logToServer(sid, `openIntranetTab cridat (shouldFocus=${shouldFocus})`);
    ...
    globalSse.addEventListener('state', async (event) => {
        try {
            ...
            if (stateData.state === 'searching' && !processingSearchSids.has(sid)) {
                // S'assegura que la pestanya està oberta i enfocada, i després processa la cerca
